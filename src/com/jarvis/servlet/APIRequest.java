...
public class APIRequest extends HttpServlet implements Constants {
	...
	protected void doGet(HttpServletRequest request,
		...
		/*
		 * Mongo mongo = new Mongo(); HashMap<String, Object> dbData = new
		 * HashMap<String, Object>();
		 * 
		 * String q = request.getParameter("q");
		 * System.out.println("JARVIS HERE " + q); String s = requestApi(q);
		 * APIResponse apiresponse = APIResponseParser.getResponseOnject(s);
		 * RuleEngine re = new RuleEngine(); JSONObject result =
		 * re.evaluateResults(apiresponse);
		 * 
		 * // TODO:parse the apiresponse List<Entity> elist =
		 * apiresponse.getEntities(); System.out.println("Before:\n" + elist);
		 * 
		 * JSONArray displayUrls = new JSONArray(); if
		 * (result.isNull("serviceType")) { // return
		 * result.get("defaultMessage"); } else {
		 * 
		 * JSONArray queries = result.getJSONArray("queries"); for (int i = 0; i
		 * < queries.length(); i++) { RootObject robject =
		 * BingRequest.requestBing(queries .getString(i)); List<Value> list =
		 * robject.getWebPages().getValue(); String displayurl =
		 * list.get(0).getDisplayUrl(); displayUrls.put(displayurl); }
		 * 
		 * } DBObject entityIntentData = new BasicDBObject(); Object[] obs =
		 * result.keySet().toArray(); for (int i = 0; i < obs.length; i++) {
		 ...
		 * 
		 * // TODO:call bing mongo.writeDataToDb("jarvis_sequoia", "123",
		 * dbData); response.getWriter().write(dbData.toString());
		 */
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			for (Iterator<Entity> iterator = elist.iterator(); iterator
					.hasNext();) {
